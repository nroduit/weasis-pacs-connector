...
* http://localhost:8080/weasis-pacs-connector/weasis?patientID=9702672&pro="weasis.user%20user2"    
  => Define a user for having its own preferences
...
It requires a web application container like Jetty, Tomcat or JBoss or the server side and an installation [Weasis (native installer)](https://nroduit.github.io/en/getting-started/#try-weasis-now) from the client side. 

Note: For installation with the dcm4chee user interface, see this [page](https://nroduit.github.io/en/getting-started/dcm4chee/).
...
Note: If Weasis is not installed on the server side, the `weasis.base.url` property in the weasis-pacs-connector [configuration](https://github.com/nroduit/weasis-pacs-connector/blob/master/etc/dcm4chee-arc/weasis-pacs-connector.properties) must be commented or set to null.  
...
Several archives can be used simultaneously in [configuration](https://github.com/nroduit/weasis-pacs-connector/blob/master/etc/dcm4chee-arc/weasis-pacs-connector.properties#L70) by defining an archive file in which the property `arc.activate` must be `true`. 
