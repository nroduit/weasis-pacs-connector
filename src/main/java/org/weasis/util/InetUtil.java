...
public class InetUtil {
    ...
    /**
     * Returns HostName resolved from request parameter value if present or from request caller ipAddress
     *
     * @param request                  is the ServletRequest interface
     * @param requestHostParameterName name of the query parameter to be used with for hostName resolution
     * @return <b>hostName</b> or <b>null</b> if host cannot be resolved.
     * @note : a non-null but empty parameter value would force not to resolve the HostName of the remote client
     */
    ...
    /**
     * Returns original Internet Protocol (IP) address of the client or last proxy that sent the request even if behind
     * a load balancer.
     *
     * @param request is the ServletRequest interface
    ...
    /**
     * Gets the host name without the service name part (domain name suffix).
     * A reverse name lookup will be performed and the result will be returned based on the system configured name lookup service.
     * <p>
     * Depending on the underlying system configuration best effort method is applied, meaning a simple textual
     * representation of the IP address may be returned.
     * <p>
     *
     * @param addr as literal IP address (IPV4/IPV6) or hostName (even in fully qualified domain name form)
    ...
    public static String getClientHostFromAddr(String addr) {
        ...
        if (isEmpty(addr))
            return null; // avoids resolving localhost loopback interface when supplied clientIP address is null
        ...
        if (!(isEmpty(clientHost))) {
            if (isDNSResolved) {
                ...
                if (clientHost.equalsIgnoreCase(clientHostFQDN))
                    ...
                    LOGGER.debug("Client HOST is [{}] with original FQDN format [{}] found from address [{}]", clientHost, clientHostFQDN, addr);
            } else
                LOGGER.debug("Client HOST address is [{}] can't be resolved by DNS", clientHost);
        }
