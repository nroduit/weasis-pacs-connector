...
public class ServletUtil {
  ...
  public static boolean isRequestAllowed(
      HttpServletRequest request, Properties archiveProperties, Logger logger) {
    ...
    if (StringUtil.hasText(hosts)) {
      ...
      if (!accept) {
        if (logger != null) {
          logger.warn(
              "The request from {} is not allowed.", clientHost.replaceAll("[\n|\r|\t]", "_"));
        }
        return false;
      }
    }
  ...
  public static void logInfo(HttpServletRequest request, Logger logger) {
    ...
    logger.debug(
        "HttpServletRequest - getRequestQueryURL: {}{}",
    ...
    logger.debug(
        ...
        request.getParameterMap().entrySet().stream()
            .flatMap(e -> Stream.of(e.getValue()).map(v -> e.getKey() + "=" + v))
    ...
    try {
      ...
      if (STUDY_LEVEL.equals(requestType) && isRequestIDAllowed(STUDY_LEVEL, properties)) {
        ...
        if (hasText(anbID)) {
          ...
          LOGGER.error(
              "No ID found for STUDY request type: {}", requestType.replaceAll("[\n|\r|\t]", "_"));
          params.addGeneralViewerMessage(
              new ViewerMessage(
                  "Missing Study ID",
                  "No study ID or AccessionNumber found in the request",
                  ViewerMessage.eLevel.ERROR));
        }

      } else if (PATIENT_LEVEL.equals(requestType)
          && isRequestIDAllowed(PATIENT_LEVEL, properties)) {
        if (!doBuildQuery) return true;
        ...
        if (hasText(patID)) {
          ...
          LOGGER.error(
              "No ID found for PATIENT request type: {}",
              requestType.replaceAll("[\n|\r|\t]", "_"));
          params.addGeneralViewerMessage(
              new ViewerMessage(
                  "Missing Patient ID",
                  "No patient ID found in the request",
                  ViewerMessage.eLevel.ERROR));
        }
        ...
        LOGGER.error(
            "Not supported IID request type: {}", requestType.replaceAll("[\n|\r|\t]", "_"));
        params.addGeneralViewerMessage(
            new ViewerMessage(
                "Unexpected Request",
                "Not supported IID request type: " + requestType,
                ViewerMessage.eLevel.ERROR));
      }
      ...
      params.addGeneralViewerMessage(
          new ViewerMessage(
              "Unexpected Error",
              "Unexpected Error when building the manifest: " + e.getMessage(),
              ViewerMessage.eLevel.ERROR));
    }
  ...
  public static ManifestBuilder buildManifest(
      HttpServletRequest request, ConnectorProperties props) {
  ...
  public static String buildManifestURL(
      HttpServletRequest request, ManifestBuilder builder, Properties props, boolean gzip) {
    StringBuilder buf =
        new StringBuilder(
            props.getProperty("manifest.base.url", props.getProperty("server.base.url")));
