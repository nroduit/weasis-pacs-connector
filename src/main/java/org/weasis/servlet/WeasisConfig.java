/*
 * Copyright (c) 2019-2021 Weasis Team and other contributors.
 *
...
public class WeasisConfig extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    properties.put(
        "weasis.i18n", codeBasePath + "-i18n"); // Works only if war files are not renamed
    ...
    properties.put(
        "felix.extended.config.properties", codeBaseExtPath + "/conf/ext-config.properties");
  ...
  private void handleRequestArgumentParameter(
  ...
  private void handleRequestPropertyParameter(
  ...
  public static String getCodebase(
      HttpServletRequest request, ConnectorProperties props, boolean extCodebase) {
    String codeBasePath;
    String queryCodeBasePath =
        request.getParameter(extCodebase ? PARAM_CODEBASE_EXT : PARAM_CODEBASE);
    if (queryCodeBasePath != null) {
      if (queryCodeBasePath.startsWith("/")) {
        codeBasePath = ServletUtil.getBaseURL(request) + queryCodeBasePath;
      } else {
        codeBasePath =
            StringUtil.hasText(queryCodeBasePath) ? queryCodeBasePath : StringUtil.EMPTY_STRING;
